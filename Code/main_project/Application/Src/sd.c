...
uint16_t parse_to_text_dose(char *buffer, rf_dosimeter_package_crc_t data) {
	char buf [500];
	uint16_t num_written = snprintf(&(buf[0]), 500, "%u;%u;%lu;%lu;%lu;%u\n", data.pack.flag, data.pack.num, data.pack.time_from_start, data.pack.ticks_per_last_second, data.pack.ticks_sum, data.crc);
...
uint16_t parse_to_text_ds(char *buffer, rf_ds_package_crc_t data) {
	char buf [500];
	uint16_t num_written = snprintf(&(buf[0]), 500, "%u;%u;%lu;%f;%u;%u\n", data.pack.flag, data.pack.num, data.pack.time_from_start, data.pack.ds18b20_temperature, data.pack.status, data.crc);
	memcpy(buffer, &buf, num_written);
	return num_written;
}
